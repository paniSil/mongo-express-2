...
В контролерах є допоміжні функції findUserByEmail, findUserById, createUserInDb, updateUserInDb, findArticleByTitle для оптимізації роботи з користувачами і статтями.
...
- **insertOne**: POST /articles — створення однієї статті (тіло: { title, text }). При створенні нової статті (/articles/new) реалізована перевірка на унікальність заголовка (повертається 409 та відображається помилка в EJS).
